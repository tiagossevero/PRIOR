...
def render_analise_geografica(dados):
    ...
    df = df_master.merge(df_score[available_score_cols], on=merge_cols, how='inner')
...
def render_analise_clusters(dados):
    ...
    st.dataframe(
        ...
        column_config={
            ...
            'empresas': 'Empresas',
            'valor_total': st.column_config.NumberColumn('Valor Total', format="R$ %.2f"),
            'dias_medio': st.column_config.NumberColumn('Dias Médio', format="%.0f"),
            'score_medio': st.column_config.NumberColumn('Score Médio', format="%.1f")
        }
    )
...
def render_outliers(dados):
    ...
    st.dataframe(
        df_top[['inscricao_estadual', 'valor_devido', 'dias_cobranca', 'score', 'qtd_outliers']],
        hide_index=True,
        column_config={
            'inscricao_estadual': 'IE',
            'valor_devido': st.column_config.NumberColumn('Valor Devido', format="R$ %.2f"),
            'dias_cobranca': 'Dias',
...
def render_analise_correlacao(dados):
    ...
    if not componentes_disponiveis:
        st.warning("Componentes de score não disponíveis")
        return
    ...
    fig = px.imshow(
        ...
        text_auto='.2f',
        aspect='auto',
        title="Matriz de Correlação entre Componentes",
        color_continuous_scale='RdBu_r',
        zmin=-1,
        zmax=1
    )
    ...
    st.markdown("---")
    st.subheader("📊 Análise de Pares de Componentes")
    
    col1, col2 = st.columns(2)
    
    if len(componentes_disponiveis) >= 2:
        with col1:
            comp1 = st.selectbox("Componente 1:", componentes_disponiveis, key='comp1')
        with col2:
            comp2 = st.selectbox("Componente 2:", componentes_disponiveis, index=1, key='comp2')
        
        if comp1 != comp2:
            fig = px.scatter(
                df_score.sample(min(1000, len(df_score))),
                x=comp1,
                y=comp2,
                title=f"Correlação: {comp1} vs {comp2}",
                trendline="ols"
            )
            st.plotly_chart(fig, use_container_width=True)
...
def render_machine_learning_avancado(dados):
    ...
    st.info("Use a seção original 'Machine Learning' no menu para funcionalidades preditivas completas")
...
def render_relatorios_executivos(dados):
    ...
    if st.button("📥 Exportar Relatório Completo (CSV)"):
        df_relatorio = df_score.merge(
            df_master[['inscricao_estadual', 'tipo_debito', 'razao_social', 'nome_municipio']],
            on=['inscricao_estadual', 'tipo_debito'],
            how='left'
        )
        ...
        st.download_button(
            "📥 Download",
            csv,
            f"relatorio_gecob_{datetime.now().strftime('%Y%m%d')}.csv",
            "text/csv"
        )
...
def render_simulador_cenarios(dados):
    ...
    st.markdown("""
    ### 🎯 Simulador de Estratégias de Cobrança
    
    Ajuste os parâmetros abaixo para simular diferentes cenários de cobrança:
    """)
    
    col1, col2 = st.columns(2)
    
    with col1:
        taxa_sucesso = st.slider("Taxa de Sucesso Esperada (%)", 0, 100, 30)
        custo_contato = st.number_input("Custo por Contato (R$)", 0.0, 1000.0, 50.0)
    
    with col2:
        prioridade_foco = st.selectbox(
            "Foco de Priorização:",
            ["PRIORIDADE_MAXIMA", "PRIORIDADE_ALTA", "PRIORIDADE_MEDIA", "PRIORIDADE_BAIXA"]
        )
        num_contatos = st.number_input("Número de Contatos", 1, 10000, 100)
    
    if st.button("🚀 Simular Cenário"):
        df_score = dados.get('score', pd.DataFrame())
        
        if not df_score.empty:
            df_foco = df_score[df_score['classificacao_prioridade'] == prioridade_foco]
            
            if len(df_foco) >= num_contatos:
                df_simulacao = df_foco.nlargest(num_contatos, 'score_final_priorizacao')
                
                valor_total_simulado = df_simulacao['valor_total_devido'].sum()
                valor_esperado = valor_total_simulado * (taxa_sucesso / 100)
                custo_total = num_contatos * custo_contato
                ...
                with col1:
                    st.metric("💰 Valor Total", f"R$ {valor_total_simulado/1e6:.2f}M")
                
                with col2:
                    st.metric("✅ Valor Esperado", f"R$ {valor_esperado/1e6:.2f}M")
                ...
                st.success(f"""
                **Análise:** Com {num_contatos} contatos focados em {prioridade_foco} 
                e taxa de sucesso de {taxa_sucesso}%, espera-se recuperar R$ {valor_esperado/1e6:.2f}M 
                com ROI de {roi:.1f}%.
                """)
            else:
                st.warning(f"Apenas {len(df_foco)} casos disponíveis nesta prioridade")
...
def render_alertas_inteligentes(dados):
    ...
    st.markdown("### 🚨 Alertas Automáticos do Sistema")
    
    # Alerta 1: Casos críticos
    criticos = df_score[df_score['classificacao_prioridade'] == 'PRIORIDADE_MAXIMA']
    if len(criticos) > 0:
        st.markdown(f'<div class="alert-danger"><strong>🔴 ALERTA CRÍTICO</strong><br>{len(criticos):,} casos de prioridade máxima requerem ação imediata!</div>', unsafe_allow_html=True)
    
    # Alerta 2: Alta concentração
    df_sorted = df_score.sort_values('valor_total_devido', ascending=False)
    top_10_pct = int(len(df_sorted) * 0.1)
    valor_top_10 = df_sorted.head(top_10_pct)['valor_total_devido'].sum()
    valor_total = df_sorted['valor_total_devido'].sum()
    ...
    if concentracao > 80:
        st.markdown(f'<div class="alert-warning"><strong>⚠️ ALERTA DE CONCENTRAÇÃO</strong><br>Top 10% dos casos concentram {concentracao:.1f}% do valor total</div>', unsafe_allow_html=True)
    
    # Alerta 3: Oportunidades
    if 'score_capacidade_pagamento' in df_score.columns and 'score_historico_pagamento' in df_score.columns:
        oportunidades = df_score[
            (df_score['score_capacidade_pagamento'] > 70) &
            (df_score['score_historico_pagamento'] > 60)
        ]
        if len(oportunidades) > 0:
            valor_oportunidade = oportunidades['valor_total_devido'].sum()
            st.markdown(f'<div class="alert-success"><strong>✅ OPORTUNIDADE IDENTIFICADA</strong><br>{len(oportunidades):,} empresas com alto potencial de pagamento (R$ {valor_oportunidade/1e6:.2f}M)</div>', unsafe_allow_html=True)
    
    # Alerta 4: Sem contato
    if 'qtd_total_contatos' in df_master.columns:
        sem_contato = df_master[df_master['qtd_total_contatos'] == 0]
        if len(sem_contato) > 0:
            valor_sem_contato = sem_contato['valor_total_devido'].sum()
            st.markdown(f'<div class="alert-info"><strong>📞 ALERTA DE CONTATO</strong><br>{len(sem_contato):,} empresas sem contato (R$ {valor_sem_contato/1e6:.2f}M)</div>', unsafe_allow_html=True)
    
    st.markdown("---")
    st.subheader("📊 Configurações de Alertas")
    st.info("Sistema de alertas automáticos configurado e ativo. Novos alertas serão exibidos conforme mudanças nos dados.")
